<template>
    <ul class="v-datatable-light-pagination" :class="css.pagination">
        <li
            v-if="moveFirstPage"
...
export default {
  ...
  computed: {
    qntPages: function () {
      ...
      if (nrPages > 4) {
        if (this.currPage <= 3) {
          return Array.apply(null, { length: 5 }).map(
            (_, index) => index + 1
          )
        ...
        return Array.apply(null, { length: nrPages }).map(
          (_, index) => index + 1
        )
      }
    },
  ...
  methods: {
    ...
    changePage: function (pageToMove) {
      if (
        pageToMove <= this.lastPage &&
                pageToMove >= 1 &&
                pageToMove !== this.currPage
      ) {
